export default function initImageResize(editor) {
  /*
   * ImageResize Plugin for Editor (http://github.com/nmmf/imageresize)
  ...
  editor.plugins.add("imageresize", {
    ...
    resizeAll: function (editor, parent, width, height) {
      ...
      /* Parent Node is a Editor DOM Node */
      if ("find" in parent && typeof parent.find == "function") {
      } else parent = new editor.dom.node(parent);
      ...
      if (parent && "find" in parent && typeof parent.find == "function") {
        ...
        for (index = 0; index < l; index++) {
          ...
          if (s && s.indexOf("data:") === 0 && (w > width || h > height))
    ...
    resize: function (editor, imageElement, width, height) {
      ...
      img.onload = function () {
        ...
        if (this[ns].n) {
          if (
            /^data:image\/jpeg/i.test(this.src) ||
            /\.(jpg|jpeg)$/i.test(this.src)
          ) {
